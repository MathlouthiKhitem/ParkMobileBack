...
public class UsersResource {
    ...
    public ResponseEntity<Users> updateUsers(@PathVariable(value = "id", required = false) final String id, @RequestBody Users users)
        ...
        // Convert the string-based ID to ObjectId
        ObjectId objectId;
        try {
            objectId = new ObjectId(id);
        } catch (IllegalArgumentException e) {
            throw new BadRequestAlertException("Invalid ID format", ENTITY_NAME, "idinvalid");
        }

        if (!usersRepository.existsById(String.valueOf(objectId))) {
